...
function buildNotificationHtml(params: {
  ...
  const { name, email, phone, country, city, availability, experience, motivation } = params;
  return `<!DOCTYPE html><html><body style="margin:0;font-family:sans-serif;background:#f5f5f5;padding:24px;">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
  ...
  <div style="padding:24px;">
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      ...
      ${city ? `<tr><td style="padding:8px 0;color:#64748b;">City</td><td style="padding:8px 0;">${escapeHtml(city)}</td></tr>` : ""}
      ${phone ? `<tr><td style="padding:8px 0;color:#64748b;">Phone</td><td style="padding:8px 0;">${escapeHtml(phone)}</td></tr>` : ""}
      ${availability ? `<tr><td style="padding:8px 0;color:#64748b;">Availability / Preferred dates</td><td style="padding:8px 0;">${escapeHtml(availability)}</td></tr>` : ""}
      ${experience ? `<tr><td style="padding:8px 0;color:#64748b;">Experience with animals</td><td style="padding:8px 0;">${escapeHtml(experience)}</td></tr>` : ""}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await delay(600);
    const notifHtml = buildNotificationHtml({
      name,
      email,
      phone,
      country,
