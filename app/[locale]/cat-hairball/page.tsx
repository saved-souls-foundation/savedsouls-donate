...
export default function CatHairballPage() {
  const t = useTranslations("catHairball");
    ...
    <ParallaxPage backgroundImage="/savedsoul-logo-bg.webp">
      <main className="max-w-4xl mx-auto px-4 py-12 md:py-16">
        <div className="mb-12 rounded-3xl overflow-hidden shadow-xl border-2 border-stone-200 dark:border-stone-600">
          <Image src="/guides/cat-hairball-hero.png" alt="" width={1200} height={630} className="w-full h-auto object-cover min-h-[280px] md:min-h-[360px]" priority />
        </div>
        ...
        <div className="mb-16 max-w-2xl mx-auto">
          <FrostDonateCard src="/guides/cat-hairball-inline.png" alt={t("img1Alt")} namespace="catHairball" />
        </div>
        ...
        <section className="mb-16 rounded-2xl p-6 md:p-10 bg-white dark:bg-stone-900 border-2 border-stone-200 dark:border-stone-700 shadow-lg">
          <h2 className="text-2xl font-bold mb-6" style={{ color: ACCENT_GREEN }}>🔬 {t("whatTitle")}</h2>
          <p className="text-stone-600 dark:text-stone-400 leading-relaxed">{t("what")}</p>
        </section>

        <div className="mb-16 rounded-2xl overflow-hidden border-2 border-stone-200 dark:border-stone-600">
          <Image src="/guides/cat-hairball-inline.png" alt="" width={600} height={600} className="w-full h-auto object-cover min-h-[240px]" />
        </div>
        ...
        <section className="mb-16 rounded-2xl p-6 md:p-10 bg-white dark:bg-stone-900 border-2 border-stone-200 dark:border-stone-700 shadow-lg">
          <h2 className="text-2xl font-bold mb-6" style={{ color: ACCENT_GREEN }}>🩺 {t("whenTitle")}</h2>
          <p className="text-stone-600 dark:text-stone-400 leading-relaxed">{t("when")}</p>
        </section>

        <section className="mb-16 rounded-2xl p-6 md:p-10 bg-white dark:bg-stone-900 border-2 border-stone-200 dark:border-stone-700 shadow-lg">
          <h2 className="text-2xl font-bold mb-4" style={{ color: ACCENT_GREEN }}>✨ {t("skinTitle")}</h2>
          <p className="text-stone-600 dark:text-stone-400 leading-relaxed mb-4">{t("skin")}</p>
          <Link href="/skin-coat" className="inline-flex font-semibold hover:underline" style={{ color: ACCENT_GREEN }}>
            {t("skinLink")} →
          </Link>
        </section>
