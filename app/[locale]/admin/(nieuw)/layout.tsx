...
export default async function AdminNieuwLayout({
  ...
  try {
    ...
    // Zelfde definitie als Onbeantwoord-tab: in_behandeling + beantwoord_op is null
    const onbeantwoordCount = admin.from("incoming_emails").select("*", { count: "exact", head: true }).eq("status", "in_behandeling").is("beantwoord_op", null);
    const onbeantwoordList = admin
      .from("incoming_emails")
      .select("id, onderwerp, van_email, van_naam, ontvangen_op")
    ...
    if (!countRes.error && !listRes.error) {
      ...
      const { count } = await admin.from("incoming_emails").select("*", { count: "exact", head: true }).eq("status", "in_behandeling").is("beantwoord_op", null);
