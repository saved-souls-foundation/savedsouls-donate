...
export default function CatIndoorOutdoorPage() {
  const t = useTranslations("catIndoorOutdoor");
    ...
    <ParallaxPage backgroundImage="/savedsoul-logo-bg.webp">
      <main className="max-w-4xl mx-auto px-4 py-12 md:py-16">
        <div className="mb-12 rounded-3xl overflow-hidden shadow-xl border-2 border-stone-200 dark:border-stone-600">
          <Image src="/guides/cat-indoor-outdoor-hero.png" alt="" width={1200} height={630} className="w-full h-auto object-cover min-h-[280px] md:min-h-[360px]" priority />
        </div>
        ...
        <section className="mb-10 rounded-2xl p-6 md:p-10 bg-gradient-to-br from-teal-50 to-cyan-50 dark:from-teal-950/30 dark:to-cyan-950/30 border-2 border-stone-200 dark:border-stone-700 shadow-lg">
          <p className="text-lg text-stone-600 dark:text-stone-400 leading-relaxed">{t("intro")}</p>
        </section>

        <div className="mb-16 max-w-2xl mx-auto">
          <FrostDonateCard src="/guides/cat-indoor-outdoor-hero.png" alt={t("img1Alt")} namespace="catIndoorOutdoor" />
        </div>
        ...
        <section className="mb-16 rounded-2xl p-6 md:p-10 bg-white dark:bg-stone-900 border-2 border-stone-200 dark:border-stone-700 shadow-lg">
          <h2 className="text-2xl font-bold mb-6" style={{ color: ACCENT_GREEN }}>⚖️ {t("prosConsTitle")}</h2>
          <p className="text-stone-600 dark:text-stone-400 leading-relaxed mb-4">{t("prosCons1")}</p>
          <p className="text-stone-600 dark:text-stone-400 leading-relaxed">{t("prosCons2")}</p>
        </section>
